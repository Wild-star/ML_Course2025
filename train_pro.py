...
from models.LSTM_pro import LSTM, save_model, load_model
...
def main():
    ...
    for run in range(n_runs):
        print(f"\n==== Run {run+1} ====")
        model_name = f"LSTMpro_run{run+1}_pred{pred_len}"
        ...
        # 保存模型
        save_model(model, path=f"LSTM_pro_pred{pred_len}/LSTM_run{run+1}_pred{pred_len}.pt")
